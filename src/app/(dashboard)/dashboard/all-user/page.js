...
const AllUser = () => {
    ...
    const fetchStatusData = useCallback(async () => {
        try {
            const res = await axios.get(`${process.env.NEXT_PUBLIC_SERVER_LINK}/user-status`);
    ...
    const fetchUsers = useCallback(async (page = 1, limitVal = 12) => {
        try {
            const res = await axios.get(`${process.env.NEXT_PUBLIC_SERVER_LINK}/alluser`, {
                params: {
                    page,
                    limit: limitVal,
            ...
            <div className='mb-5'>
                <div className="breadcrumbs text-sm mb-6">
                    <ul>
                        ...
                        <li><Link className="text-[#03b00b]" href={"/dashboard/all-user"}>All User</Link></li>
                    </ul>
                </div>
            </div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className="avatar">
                                                <div className="mask mask-squircle h-12 w-12">
                                                    <img src={user?.photoURL || "/default-user.png"} alt={user?.name} referrerPolicy='no-referrer' />
                                                </div>
                                            </div>
                                            <div>
